...
class Cozmo(Client):
    ...
    def _start_camera(self):
        pkt = protocol_encoder.EnableCamera()
        self.conn.send(pkt)
        pkt = protocol_encoder.EnableColorImages()
        self.conn.send(pkt)
    ...
    def _init_ros2(self, node_name, namespace):
        self.node = rclpy.create_node(
            node_name=node_name, namespace=namespace or node_name
        )
        ...
        self._odom_msg = Odometry(
            header=Header(frame_id=self._odom_frame),
        ...
        # Subscribers
        self._twist_sub = self.node.create_subscription(
            Twist, "cmd_vel", self._control, 10
        )
    ...
    def _pub_odom(self, now):
        ...
        self._odom_msg.pose.pose.orientation = euler_to_quaternion(
            0.0, self.pose_pitch.radians, self.pose_angle.radians
        )
        self._odom_msg.pose.covariance = np.diag(
            [1e-2, 1e-2, 1e-2, 1e3, 1e3, 1e-1]
        ).ravel()
        # self._odom_msg.twist.twist.linear.x  = self._lin_vel
        # self._odom_msg.twist.twist.angular.z = self._ang_vel
        self._odom_msg.twist.covariance = np.diag(
            [1e-2, 1e3, 1e3, 1e3, 1e3, 1e-2]
        ).ravel()
