...
from ros2cozmo.custom_thread import CustomThread


class SpeechToTextNode(Node):  # pylint: disable=too-many-instance-attributes
    """Speech To Text Node Class"""
    ...
    def __init__(self, node_name):
        ...
        self.declare_parameter(service_param_name, "google")
        ...
        self.__pub = self.create_publisher(String, "/transcription", 10)
            ...
            with self.__mic as source:
                ...
                try:
                    if self.service == "sphinx":
                        value = self.__rec.recognize_sphinx(audio)
                    ...
                    stt_result.data = (
                        "Couldn't request results from Google STT; " + str(error)
                    )
...
def main(args=None):
    ...
    node = SpeechToTextNode(node_name="speech_to_text")
